# DỰ ÁN QUẢN LÝ QUÁN NET
Dự án làm Winform để tốt nghiệp môn Lập trình Winform, thế thôi!
## Cài đặt và Yêu cầu

Hướng dẫn cài đặt và yêu cầu cần thiết để chạy dự án trên máy tính của bạn.

## Cấu trúc thư mục

Mô tả cấu trúc thư mục của dự án và vai trò của từng thư mục.

## Hướng dẫn sử dụng

Hướng dẫn chi tiết về cách sử dụng dự án. Bao gồm các bước để chạy ứng dụng và sử dụng các tính năng cụ thể.

## Công nghệ sử dụng

- .Net Framework 4.8
- SQL Server (LocalDB)
- Công nghệ lập trình chạy bằng cơm và nước tương

## Tính năng quan trọng

- Quản lý hệ thống Client-Server side trực quan
- Kết nối giữa Server-Client thông qua SocketIO
- Giao diện tường minh, thân thiện với design các quán net

## Thành viên

| Tên Thành Viên        | Tên Github      | Phần Trăm Tham Gia | Công Việc Đóng Góp                            | Phần Trăm Dự Án | Phần Trăm Hoàn Thành |
|----------------------|-----------------|-------------------|----------------------------------------------|-----------------|---------------------|
| Đặng Quang Hiển      | Miho1254        | 0%                | Quản lý source code, giao Task và Design Source code | 0%              | 0%                  |
| Lê Đình Hiếu         | meotrang3051    | 0%                | Quản lý Frontend                             | 0%              | 0%                  |
| Trần Thị Hồng Ngọc   | vmxam           | 0%                | Quản lý Backend                              | 0%              | 0%                  |
| Trần Tuấn Kiệt       | sauring123      | 0%                | Hỗ trợ phần Backend với Trần Thị Hồng Ngọc    | 0%              | 0%                  |
| Nguyễn Quốc Bảo      | Unknown         | 0%                | Hỗ trợ phần Frontend với Lê Đình Hiếu         | 0%              | 0%                  |

## Đường dẫn
- [Quản lý phân công](https://github.com/Miho1254/project_QuanLyQuanNet/blob/master/Documentation/Task.md)
- [Sơ đồ Use Case](https://github.com/Miho1254/project_QuanLyQuanNet/blob/master/Documentation/UseCase.md)
- [Cấu trúc CSDL](https://github.com/Miho1254/project_QuanLyQuanNet/blob/master/Documentation/CSDL.jpg)
- [Bản thiết kế Form](https://github.com/Miho1254/project_QuanLyQuanNet/blob/master/Documentation/Form_Design_Blueprint.md)
